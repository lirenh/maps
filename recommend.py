...
def group_by_centroid(restaurants, centroids):
    ...
    """
    #"*** YOUR CODE HERE ***"
    l = [[find_closest(restaurant_location(r), centroids), r] for r in restaurants]
...
def find_centroid(restaurants):
    ...
    #"*** YOUR CODE HERE ***"
    lat = [restaurant_location(r)[0] for r in restaurants]
    long = [restaurant_location(r)[1] for r in restaurants]
...
def find_predictor(user, restaurants, feature_fn):
    ...
    xys = zip(xs, ys)
    S_xx = sum([pow((x-mx), 2) for x in xs])
    #print("S_xx", S_xx)
    S_yy = sum([pow((y-my), 2) for y in ys])
    #print("S_yy", S_yy)
    S_xy = sum([(x-mx)*(y-my) for x, y in xys])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    #"*** YOUR CODE HERE ***"
    best_feature = max(feature_fns, key=lambda f: find_predictor(user, reviewed, f)[1])
...
def rate_all(user, restaurants, feature_functions):
    ...
    for name in restaurants.keys():
        if name in reviewed:
            ...
            new_dict[name] = predictor(restaurants[name])
    return new_dict
